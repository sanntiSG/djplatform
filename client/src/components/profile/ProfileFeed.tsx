...
export function ProfileFeed({ profile, events, onItemClick }: ProfileFeedProps) {
  ...
  return (
    <div ref={gridRef} className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-1.5 md:gap-2">
      {feed.map((item, i) => (
        <div
          key={`${item.kind}-${i}`}
          className="feed-tile relative aspect-square overflow-hidden rounded-xl bg-[var(--surface-elevated)] cursor-pointer group"
          onClick={(e) => {
