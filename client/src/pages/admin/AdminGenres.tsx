...
import { useAdminGenres, useCreateGenre, useUpdateGenre, useDeleteGenre } from '../../hooks/useCatalogs.js'
import { Button } from '../../components/ui/Button.js'
import { Input } from '../../components/ui/Input.js'
import { cn } from '../../utils/cn.js'
import { prefersReducedMotion } from '../../utils/motion.js'
import type { AdminGenreItem } from '../../services/catalogService.js'

export default function AdminGenres() {
  const { data: genres, isLoading } = useAdminGenres()
  const { mutateAsync: createGenre, isPending: creating } = useCreateGenre()
  ...
  useEffect(() => {
    if (!listRef.current || !genres?.length || prefersReducedMotion()) return
    const rows = listRef.current.querySelectorAll<HTMLElement>('.genre-row')
  ...
  async function handleToggle(genre: AdminGenreItem) {
    try {
      await updateGenre({ id: genre.id, patch: { isActive: !genre.isActive } })
    } catch {
      // silent — query will re-sync
    }
  }
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        <h2 className="font-display font-semibold text-[var(--text)] text-xl">Generos musicales</h2>
        <p className="font-sans text-sm text-[var(--text-muted)] mt-1">
          Los generos activos aparecen en el MultiSelect de perfiles y tracks. Los inactivos quedan ocultos.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <p className="font-sans text-sm text-[var(--text-muted)]">No hay generos todavia.</p>
      ) : (
        <div ref={listRef} className="flex flex-col gap-2">
          {genres.map((g) => (
            <div
              key={g.id}
              data-id={g.id}
              className="genre-row flex items-center gap-3 px-4 py-3 rounded-xl border border-[var(--border)] bg-[var(--surface-elevated)]"
            >
              <div className="flex-1 min-w-0">
                <span className="font-sans text-sm font-medium text-[var(--text)]">{g.name}</span>
                <span className="font-sans text-xs text-[var(--text-muted)] ml-2">{g.slug}</span>
              </div>
